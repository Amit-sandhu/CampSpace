...
function WorkspaceDashboard() {
  ...
  // 1. Live 12-hour Clock Effect (HH:MM AM/PM)
  useEffect(() => {
    const updateClock = () => {
      const now = new Date();
  ...
  // 2. Weather & Reverse Geocoding Fetch Handler
  const handleShowWeather = async () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          // Fetch weather and city/state via reverse geocoding concurrently
          const [weatherRes, geoRes] = await Promise.all([
            fetch(`https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&current_weather=true`),
            fetch(`https://geocoding-api.open-meteo.com/v1/search?name=auto&count=1&latitude=${latitude}&longitude=${longitude}`) // fallback or use nominatim
          ]);

          // Alternatively, use Nominatim for accurate City, State reverse geocoding
          ...
          setWeatherError('Failed to fetch weather or location data.');
          setWeatherLoading(false);
        }
      },
      (err) => {
  ...
  <header className="mainpage-nav">
    ...
    <div className="mainpage-nav-left">
      ...
      {/* --- 12-HR CLOCK (HH:MM AM/PM) --- */}
      <div style={{ color: '#FAFAFB', fontSize: '13px', fontWeight: 500, marginLeft: '12px', opacity: 0.85, whiteSpace: 'nowrap' }}>
        {timeString}
      </div>
  ...
  {/* --- MACOS STYLE WEATHER MODAL POPUP --- */}
  {showWeatherModal && (
    <div style={{
      ...
      <div style={{
        background: '#1e1e1e', color: '#f5f5f7', padding: '0', borderRadius: '10px',
        ...
        <div style={{
          ...
          background: '#2d2d2d', borderBottom: '1px solid rgba(255,255,255,0.08)'
        }}>
          <div style={{ display: 'flex', gap: '6px' }}>
            <span onClick={() => setShowWeatherModal(false)} style={{ width: '12px', height: '12px', background: '#ff5f56', borderRadius: '50%', display: 'inline-block', cursor: 'pointer' }}></span>
          ...
          <div style={{ flex: 1, textAlign: 'center', fontSize: '12px', fontWeight: 600, color: 'rgba(255,255,255,0.85)' }}>
            Weather
          </div>
        </div>
        ...
        <div style={{ padding: '20px' }}>
          {weatherLoading && (
            <p style={{ fontSize: '13px', opacity: 0.7, textAlign: 'center', margin: '10px 0' }}>
              Detecting location & fetching weather...
            </p>
          )}
          {weatherError && (
            <p style={{ color: '#ff6b6b', fontSize: '13px', textAlign: 'center', margin: '10px 0' }}>
              {weatherError}
            </p>
          )}
          {weatherData && (
            <div>
              <div style={{ fontSize: '15px', fontWeight: 600, marginBottom: '14px', color: '#fff', textAlign: 'center' }}>
                📍 {locationName || 'Current Location'}
              </div>
              <div style={{ background: 'rgba(255,255,255,0.05)', padding: '14px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.06)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '14px' }}>
                  <span style={{ opacity: 0.7 }}>Temperature</span>
                  <span style={{ fontWeight: 600 }}>{weatherData.temperature}°C</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '14px' }}>
                  <span style={{ opacity: 0.7 }}>Wind Speed</span>
                  <span style={{ fontWeight: 600 }}>{weatherData.windspeed} km/h</span>
                </div>
              </div>
            </div>
          )}
          <button 
            ...
            style={{
              marginTop: '18px', width: '100px', padding: '6px 12px', margin: '18px auto 0 auto', display: 'block',
              background: 'rgba(255,255,255,0.1)', color: '#fff', border: '1px solid rgba(255,255,255,0.15)',
              borderRadius: '6px', cursor: 'pointer', fontSize: '13px', fontWeight: 500, textAlign: 'center'
            }}
          >
            OK
          </button>
